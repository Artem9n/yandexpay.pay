...
(function (exports) {
	  ...
	  babelHelpers.createClass(Payment, [{
	    ...
	    value: function createPayment(node, paymentData) {
	      ...
	      YaPay$1.createPayment(paymentData, {
	        ...
	        payment.on(YaPay$1.PaymentEventType.Abort, function onPaymentAbort(event) {});
	      })["catch"](function (err) {
	        console.log({
	...
	var Proxy = /*#__PURE__*/function () {
	  ...
	  babelHelpers.createClass(Proxy, [{
	    key: "getOption",
	    value: function getOption(name) {
	...
	var RestProxy = /*#__PURE__*/function (_Proxy) {
	  babelHelpers.inherits(RestProxy, _Proxy);

	  function RestProxy() {
	    babelHelpers.classCallCheck(this, RestProxy);
	    return babelHelpers.possibleConstructorReturn(this, babelHelpers.getPrototypeOf(RestProxy).apply(this, arguments));
	  }

	  babelHelpers.createClass(RestProxy, [{
	    key: "bootstrap",
	    value: function bootstrap() {
	      var _this = this;

	      this.getButtonData().then(function (result) {
	        if (result.status === 'fail') {
	          throw new Error(result.reason);
	        }

	        _this.combineOrderWithData(result.data);

	        _this.createPayment(_this.cart.element, _this.cart.paymentData);
	      })["catch"](function (error) {});
	    }
	  }, {
	    key: "getButtonData",
	    ...
	    value: function createPayment(node, paymentData) {
	      ...
	      YaPay.createCheckout(paymentData, {
	        agent: {
	          name: "CMS-Bitrix",
	          version: "1.0"
	        }
	      }).then(function (payment) {
	        _this2.cart.removeLoader();

	        _this2.mountButton(node, payment);

	        payment.on(YaPay.CheckoutEventType.Success, function (event) {
	          _this2.authorize(event).then(function (result) {
	            if (result.status === 'success') {
	              setTimeout(function () {
	                window.location.href = result.data.redirect;
	              }, 1000);
	            } else {
	              _this2.cart.showError('authorize', result.reasonCode, result.reason);
	            }
	          });
	        ...
	        payment.on(YaPay.CheckoutEventType.Error, function (event) {
	          console.log("Process", event);
	        });
	      })["catch"](function (err) {
	        node.remove();

	        _this2.cart.showError('yapayPayment', 'payment not created', err);
	      });
	    }
	  }, {
	    key: "authorize",
	    value: function authorize(event) {
	      var data = {
	    ...
	    key: "bindDebug",
	    value: function bindDebug(payment) {
	      for (var key in YaPay.CheckoutEventType) {
	        if (!YaPay.CheckoutEventType.hasOwnProperty(key)) {
	          continue;
	        }
	    ...
	    key: "combineOrderWithData",
	    value: function combineOrderWithData(data) {
	      var cart = this.cart.paymentData.cart;
	      var exampleData = {
	    ...
	    value: function changeOffer(newProductId) {
	      ...
	      if (productId !== newProductId) {
	        // todo in items
	        this.cart.widget.setOptions({
	          productId: newProductId
	        });
	        this.getButtonData().then(function (result) {
	          _this3.combineOrderWithData(result.data);
	        });
	      }
	    }
	  }, {
	    key: "changeBasket",
	    value: function changeBasket() {
	      var _this4 = this;

	      this.getButtonData().then(function (result) {
	        _this4.combineOrderWithData(result.data);
	      });
	    }
	  }]);
	  return RestProxy;
	...
	var SiteProxy = /*#__PURE__*/function (_Proxy) {
	  babelHelpers.inherits(SiteProxy, _Proxy);

	  function SiteProxy() {
	    babelHelpers.classCallCheck(this, SiteProxy);
	    return babelHelpers.possibleConstructorReturn(this, babelHelpers.getPrototypeOf(SiteProxy).apply(this, arguments));
	  }

	  babelHelpers.createClass(SiteProxy, [{
	    key: "bootstrap",
	    value: function bootstrap() {
	      var _this = this;
	    ...
	    value: function createPayment(node, paymentData) {
	      ...
	      YaPay.createPayment(paymentData, {
	        ...
	        _this2.cart.mountButton(node, payment);
	          ...
	          _this2.orderAccept(event).then(function (result) {
	            ...
	            if (!_this2.isPaymentTypeCash(event)) {
	              _this2.notify(result, event).then(function (result) {
	                if (result.success === true) {
	                  _this2.cart.widget.go(result.state, result);

	                  payment.complete(YaPay.CompleteReason.Success);
	                } else {
	                  _this2.cart.widget.go('error', result);
	            ...
	            _this2.cart.showError('yapayProcess', '', error); // todo test it
	        ...
	        payment.on(YaPay.PaymentEventType.Error, function (event) {
	          _this2.cart.showError('yapayError', 'service temporary unavailable');
	        ...
	        payment.on(YaPay.PaymentEventType.Change, function (event) {
	          ...
	          if (event.shippingAddress) {
	            _this2.getShippingOptions(event.shippingAddress).then(function (result) {
	              payment.update({
	          ...
	          if (event.pickupBounds) {
	            _this2.getPickupOptions(event.pickupBounds).then(function (result) {
	              payment.update({
	                pickupPoints: result
	              });
	            });
	          ...
	          if (event.pickupPoint) {
	            payment.update({
	              order: _this2.combineOrderWithPickupShipping(event.pickupPoint)
	            });
	          }
	        });
	      })["catch"](function (err) {
	    ...
	    value: function orderAccept(event) {
	      ...
	      var data = _objectSpread$1(_objectSpread$1({}, orderData), delivery);
	    ...
	    value: function combineOrderWithPickupShipping(pickupOption) {
	      var order = this.cart.paymentData.order;
	      return _objectSpread$1(_objectSpread$1({}, order), {}, {
	        items: [].concat(babelHelpers.toConsumableArray(order.items), [{
	          type: 'SHIPPING',
	          label: pickupOption.label,
	          amount: pickupOption.amount
	        }]),
	        total: _objectSpread$1(_objectSpread$1({}, order.total), {}, {
	    ...
	    value: function combineOrderWithProducts(products) {
	      ...
	      var exampleOrder = _objectSpread$1(_objectSpread$1({}, order), {}, {
	...
	var AbstractCart = /*#__PURE__*/function (_AbstractStep) {
	  babelHelpers.inherits(AbstractCart, _AbstractStep);

	  function AbstractCart() {
	    babelHelpers.classCallCheck(this, AbstractCart);
	    return babelHelpers.possibleConstructorReturn(this, babelHelpers.getPrototypeOf(AbstractCart).apply(this, arguments));
	  }

	  babelHelpers.createClass(AbstractCart, [{
	    key: "render",
	    value: function render(node, data) {
	      this.isBootstrap = false;
	      this.element = node;
	      this.paymentButton = null;
	      this.proxy = this.getOption('isRest') ? new RestProxy(this) : new SiteProxy(this);
	    ...
	    key: "delayChangeBasket",
	    value: function delayChangeBasket() {
	    ...
	    value: function changeBasket() {
	      ...
	      (_this$proxy = this.proxy) === null || _this$proxy === void 0 ? void 0 : _this$proxy.changeBasket();
	    }
	  }, {
	    key: "changeOffer",
	    value: function changeOffer(newProductId) {
	      ...
	      (_this$proxy2 = this.proxy) === null || _this$proxy2 === void 0 ? void 0 : _this$proxy2.changeOffer(newProductId);
	    }
	  }, {
	...
	babelHelpers.defineProperty(AbstractCart, "defaults", {
